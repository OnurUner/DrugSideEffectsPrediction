...
class DrugLogisticNet(BaseDeepNet):
	...
	def create_model(self):
		...
		for i in range(self.task_count):
			self.outputs.append(Dense(1, activation='sigmoid', kernel_initializer='glorot_normal', bias_initializer='glorot_normal', name=self.output_names[i])(inputs))
	...
	def fit(self, x_train, y_train_dict, epochs=30, batch_size=30, is_balanced=False, verbose=0):
		...
		if is_balanced:
			...
			self.model.fit(x_train, y_train_dict, batch_size=batch_size, epochs=epochs, verbose=verbose, class_weight=weigths)
		else:
			self.model.fit(x_train, y_train_dict, batch_size=batch_size, epochs=epochs, verbose=verbose)
		return self.model
